...
const lispCallback = (payload, context, hooks = []) => {
  ...
  switch (context) {
    case "paneVisible":
      ...
      console.log("todo -- add icon to controller:", icon, command, parameter_one, parameter_two); // ...command will now run below
    ...
    case "paneHidden":
      ...
      console.log("todo -- remove icon to controller:", icon, command, parameter_one, parameter_two); // ...command will now run below
  ...
  switch (command) {
    ...
    case "goto":
      ...
      if (parameter_one === "storyFragment" && typeof parameter_two === "string") hooks.hookGoto(`/${parameter_two}`);
      break;

    default:
      console.log("MISS on helpers.js lispCallback:", context, command, parameter_one, parameter_two, payload, lisp_data);
  }
};
...
export { MarkdownParagraph, InjectGatsbyBackgroundImage, InjectGatsbyBackgroundVideo, InjectSvg, InjectSvgShape, StyledWrapperDiv, StyledWrapperSection, PaneFragment, getStoryStepGraph, InjectCssAnimation, lispCallback, getVisiblePane, getScrollbarSize };
