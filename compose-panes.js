...
const ComposedPane = data => {
  ...
  .sort((a, b) => a?.field_zindex > b?.field_zindex ? 1 : -1).map((pane_fragment, index) => {
    ...
    switch (pane_fragment?.internal?.type) {
      case "paragraph__markdown":
        ...
        if (shape && !pane_fragment?.field_modal) {
          ...
          // this is a modal
          let options = payload?.modal && payload?.modal[pane?.id] && payload?.modal[pane?.id][pane_fragment?.id] && payload?.modal[pane?.id][pane_fragment?.id][viewportKey],
              this_modal_payload = {},
              this_fragment,
              this_shape,
              this_css,
      ...
      case "paragraph__background_pane":
        if (payload?.codeHooks?.hasOwnProperty(pane?.id) && payload?.codeHooks[pane?.id]?.hasOwnProperty(pane_fragment?.id)) this_payload.codeHooks = payload.codeHooks[pane.id][pane_fragment.id];
        shape = thisViewportValue(viewportKey, {
        ...
        if (typeof shape === "string" && shape !== "none") tempValue = SvgShape(shape, this_options);
    ...
    if (this_pane_fragment_type) {
      tempValue = InjectPaneFragment(tractStackFragment, this_payload?.codeHooks && "CodeHook" || this_pane_fragment_type);
    ...
    } else console.log("ERROR in compose-panes.js: pane fragment type not found.");
